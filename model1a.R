...
#simple model
model1a  <- lme(H1a, data = final_data , random = ~1 | id / Reliability,method = "ML", na.action = na.exclude)

#complex model: add variance or correlation structure to the simple model

#variance structure: 
model1a_varIdent  <- lme(H1a, data = final_data , random = ~1 | id / Reliability,method = "ML", na.action = na.exclude,weights=varIdent(form = ~1 | Reliability))
#correlation structure:
model1a_AR1 <- lme(H1a, data = final_data , random = ~1 | id / Reliability,method = "ML", na.action = na.exclude,correlation = corAR1())
model1a_corCompSymm  <- lme(H1a, data = final_data , random = ~1 | id / Reliability,method = "ML", na.action = na.exclude, correlation = corCompSymm())
model1a_corSymm <- lme(H1a, data = final_data, random = ~1 | id / Reliability, 
...
model_names <- c(
  "model1a simple",
  "model1a with varIdent",
  "model1a with AR1",
  "model1a with corCompSymm",
  "model1a with corSymm"
)
...
a1 <- anova(model1a,model1a_corSymm)
a2 <- anova(model1a_varIdent,model1a_corSymm)
a3 <- anova(model1a_AR1,model1a_corSymm)
a4 <- anova(model1a_corCompSymm,model1a_corSymm)

best_model_table <- rbind(a1,a2,a3,a4)

compare_performance(  model1a,
                      model1a_varIdent,
                      model1a_AR1,
                      model1a_corCompSymm,
                      model1a_corSymm,
...
criteria_model1a <- cbind(model_names,as.data.frame(results))
rownames(criteria_model1a) <- model_names
print(criteria_model1a)


# Export tables -----------------------------------------------------------

current_date <- Sys.Date()  # Get today's date
filename1 <- paste0( current_date, "anova", ".xlsx")  # Dynamic filename
filename2 <- paste0(current_date, "Criteria",  ".xlsx")  # Dynamic filename
filename3 <- paste0(current_date, "Super_anova_superior_model",  ".xlsx")  # Dynamic filename

# Specify the save location
#save_location <- (r"(G:\Meine Ablage\Masterarbeit\Data Analysis\R_script_of_ME\Results_tables)")  # Replace with your desired folder path
...
write_xlsx(criteria_model1a, file_path2)
write_xlsx(anova_results_model1a, file_path1)
write_xlsx(best_model_table, file_path3)

#
filename <- paste0( current_date,"anova_part", ".xlsx") 
file_path <- file.path(save_location, filename)
anova_table <- anova(model1a, model1a_varIdent)
#write_xlsx(anova_table, file_path)

# Remove 5% outliers Residual_Pearson --------------------------------------------------------
model <- model1a_corSymm
summary(model)
...
model1a_corSymm_new<- lme(H1a, data = final_data_clean, random = ~1 | id / Reliability, 
                          method = "ML", na.action = na.exclude, correlation = corSymm(), 
                          control = control)
new_model <- model1a_corSymm_new
...
print(fixed_ci) 
filename_CI <- paste0( current_date, "CI95_model1a_before",".xlsx") 
...
# Print or save the table
print(new_fixed_ci) 
new_filename_CI <- paste0(current_date, "CI95_model1a_after",  ".xlsx") 
...
# Print or save the table
filename_FE <- paste0(current_date, "fixed_effects_table_model1a", ".xlsx") 
...
print(fixed_effects_table)
# Print or save the table
new_filename_FE <- paste0(current_date, "new_fixed_effects_table_model1a", ".xlsx") 
